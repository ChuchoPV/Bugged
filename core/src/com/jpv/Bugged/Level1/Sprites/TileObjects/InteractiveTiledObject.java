...
public abstract class InteractiveTiledObject {
    ...
    public InteractiveTiledObject(PlayScreen screen, MapObject object){
        ...
        bdef.position.set((bounds.getX() + bounds.getWidth() / 2) / LevelManager.PPM, (bounds.getY() + bounds.getHeight() / 2) / LevelManager.PPM);

        body = world.createBody(bdef);

        shape.setAsBox(bounds.getWidth() / 2 / LevelManager.PPM, bounds.getHeight() / 2 / LevelManager.PPM );
        fdef.shape = shape;
